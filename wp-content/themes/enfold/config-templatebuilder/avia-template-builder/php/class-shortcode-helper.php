...
if( ! class_exists( 'ShortcodeHelper' ) )
{
	class ShortcodeHelper
	{
		 ...
		 **/
		static public function allowed_shortcodes( $params, $remove = false )
	 	{
	 		if( ! $remove )
	 		{
		...
		/**
		 * Checks if the current element is nested within column or section
		 *
		 * @return boolean
		 */
		static public function is_top_level()
		{
			global $avia_config;

			if( ! isset( $avia_config['conditionals']['is_builder'] ) )
			{
				return false;
			}

			if( isset($avia_config['current_column'] ) || isset( $avia_config['layout_container'] ) )
			{
				return false;
			}
		 ...
		 * @return string
		 **/
		static public function build_pattern( $predefined_tags = false )
	 	{
		 ...
		 * @return string
		 **/
		static public function get_fake_pattern( $nested = false, $shortcode_tags = false, $return = 'fake' )
	 	{
...
//			}
		 ...
		 *
		 * @param int $index
		 * @param int|false $sibling
		 * @param array|false $tree
		 * @return array|false
		 */
		static public function find_tree_item( $index, $sibling = false, $tree = false )
		{
			if( empty( self::$tree ) )
			{
				return false;
...
*/
         ...
         */
        static public function avia_remove_autop( $content, $do_shortcode = false )
        {
            $shortcode_tags = array_merge( ShortcodeHelper::$allowed_shortcodes, ShortcodeHelper::$nested_shortcodes );
            $tagregexp = join( '|', array_map( 'preg_quote', $shortcode_tags ) );
         ...
         * @return string $content
         */
        static public function avia_apply_autop( $content, $do_shortcode = true )
        {
		...
		static public function create_shortcode_by_array( $name, $content = null, $args = array() )
		{
			...
			//  important: add 2 linebreaks otherwise WP will add a <br> tag and break frontend layout ( wpautop() ) !!!
			$sc .= "\n\n";
