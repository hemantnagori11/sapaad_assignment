...
//			$output .=			"<a class='avia-new-target'  href='#new-target' title='" . __( 'Move Section','avia_framework' ) . "'>+</a>";
		...
		protected function get_element_styles( array $args )
		{
			...
			{
				$element_styling->add_styles( $selector_background, array( 'background-color' => $atts['custom_bg'] ) );
			}
			...
			if( ! empty( $atts['margin'] ) )
			{
				...
				{
					$top = $explode_custom_margin['0'];
					$bottom = $explode_custom_margin['1'];
				}
				else
				{
					$top = $explode_custom_margin['0'];
					$bottom = $explode_custom_margin['0'];
				}
				...
				$element_styling->add_styles( 'section-outer', $styles );
			}
